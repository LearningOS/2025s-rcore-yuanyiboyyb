...
impl Inode {
    ...
    pub fn delete_hard_link(&self,name:&str)->isize{
        ...
        let node_id = self.read_disk_inode(op);
        if !node_id.is_some() {
            return -1;
        }
        let old_inode =  self.find(name).unwrap();
        let count = old_inode.modify_disk_inode(|disknode|{
            disknode.ref_count-=1;
            disknode.ref_count
        });
        if count == 0{
            old_inode.clear();
            old_inode.clear_bitmap_inode(node_id.unwrap());
        }
        let op1 = |root_inode: &mut DiskInode| {
            self.delete_inode_enrty(name, root_inode)
        };
        self.modify_disk_inode(op1);
        0
    }
    /// Create hard link entry
    pub fn create_hard_link(&self,old_name:&str,new_name:&str) -> isize{
        ...
        let node_id = self.read_disk_inode(op1);
        if !node_id.is_some() {
            return -1;
        }
        ...
        self.modify_disk_inode(|root_inode| {
            ...
            // write dirent
            let dirent = DirEntry::new(new_name, node_id.unwrap());
            root_inode.write_at(
    ...
    ///
    pub fn clear_bitmap_inode(&self,node_id:u32){
        let mut fs = self.fs.lock();
